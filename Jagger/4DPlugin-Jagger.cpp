      ...
      for (char line[BUF_SIZE]; std::fgets (line, BUF_SIZE, fp); ) {
        ...
        if (std::strncmp (line, "EOS\n", 4) == 0) {
          for (size_t i (0), j (0), ti (0), ti_prev (0); j < ss.size (); i += ss[j].first, ti_prev = ti, ++j) {
            ...
            if (n >= num_seed && check_ctype (&cs[i], shift, &char_t[0]) != NUM) { // for unseen tokens
    ...
    { // pruning patterns
      ...
      for (bag_t <std::pair <std::string, int> >::iter it = pbag.begin (); it != pbag.end (); ++it) {
        ...
        if (pi2sfic[pi].empty ()) { // unseen patterns
          ...
          } else if  (check_ctype (c.c_str (), shift, &char_t[0]) == NUM)
            fi = _fbag.to_i (std::string (FEAT_NUM) + ",*,*,*\n");
          else if (check_ctype (c.c_str (), shift, &char_t[0]) != OTHER)
        ...
        const int ctype = check_ctype (c.c_str (), shift, &char_t[0], shift ? ANY : OTHER); // NUM -> OTHER
...
static void getDefaultDict(std::string &defaultdict) {
    
    getResourceDir(defaultdict);
    
    defaultdict += ("dict" SEP "ipadic" SEP "dict");
}
...
void Jagger_train(PA_PluginParameters params) {
    ...
    std::string m, d, u, train;
    std::vector <std::string> dict;
    
    if(!file_object_to_path(PA_GetObjectParameter(params, 4), d)) {
        getDefaultDict(d);
    }
    
    dict.insert(dict.begin(), d);
    
    if(   (folder_object_to_path(PA_GetObjectParameter(params, 1), m))
        &&(  file_object_to_path(PA_GetObjectParameter(params, 3), u))
        &&(  file_object_to_path(PA_GetObjectParameter(params, 2), train))) {
